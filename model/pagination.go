...
func (p *Pagination) PerPage() int64 {
	return p.perPage
}

// Model(*Q(&User{}, data, []Sort{}, 1, false)).Paginate(c, perPage)
func (bm *Model) Paginate(model interface{}, c Context, perPage int64) (pagination Pagination, err error) {
	...
	_p, err := strconv.ParseUint(p, 10, 64)
	if err != nil {
		return pagination, err
	}
	page := int64(_p)
	...
	// current page num
	pagination.currentPageNum = uint(page)
