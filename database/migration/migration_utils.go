...
func (mu *MigrationUtils) currentBatch() int64 {
	migration := &Migration{}
	mu.DB().Order("batch desc").First(&migration)

	if migration.Batch > 0 {

		return int64(migration.Batch)
	}
	return 0
}
func (mu *MigrationUtils) addMigration(migratorName string, batch int64) bool {
...
func (mu *MigrationUtils) needRollbackMigrationList(batch int64) (migrationList []Migration) {
