...
func (m *memoryBasic) Put(key string, value interface{}, future time.Duration) bool {
	k := NewKey(key, m.Prefix())

	m.cache.Set(k.Prefixed(), m.parseValue(value), future)

	//@todo Event KeyWritten
	return true
}
func (m *memoryBasic) Add(key string, value interface{}, future time.Duration) bool {
	k := NewKey(key, m.Prefix())

	// if exist or expired return false
	if err := m.cache.Add(k.Prefixed(), m.parseValue(value),(future)); err != nil {
		return false
	}
